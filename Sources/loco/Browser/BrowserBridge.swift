...
final class BrowserBridge {
    ...
    func selection(appName: String) -> RawSelection? {
        let js = "(function(){try{var el=document.activeElement;if(!el||!el.isContentEditable){return '';}var s=window.getSelection();if(!s||!s.rangeCount||s.isCollapsed){return '';}var r=s.getRangeAt(0);var e=el.getBoundingClientRect();var rc=r.getBoundingClientRect();var selText=s.toString();var multiline=selText.indexOf('\n')>=0;var full=el.textContent||'';var pre=document.createRange();pre.selectNodeContents(el);pre.setEnd(r.startContainer,r.startOffset);var start=pre.toString().length;var end=start+selText.length;var enders='.!?\n';var a=start;while(a>0&&enders.indexOf(full.charAt(a-1))<0){a--;}while(a<end&&full.charCodeAt(a)<=32){a++;}var b=end;while(b<full.length&&enders.indexOf(full.charAt(b-1))<0){b++;}var sentence=full.slice(a,b).trim();return JSON.stringify({text:selText,sentence:sentence,multiline:multiline,x:Math.round(rc.left-e.left),y:Math.round(rc.top-e.top),width:Math.round(rc.width),h:Math.round(rc.height)});}catch(x){return '';}})()"
